...
class STFTDataLoader(BaseDataLoader):
    ...
    def __init__(self, batch_size, shuffle=False, validation_split=0.0, num_workers=1, window_length=256, overlap=0.75,
                 sampling_rate=8e3, num_segments=8, type="conv"):
        self.dataset = EdinburghDataset(window_length, overlap, sampling_rate, num_segments, type)
...
class EdinburghDataset(Dataset):
    ...
    def __init__(self, window_length, overlap, sampling_rate, num_segments, type):
        self.type = type
        if type == "rnn":
            self.data_dir = rnnpr.get_directory_name(window_length, overlap, sampling_rate, num_segments)
        else:
            self.data_dir = pr.get_directory_name(window_length, overlap, sampling_rate, num_segments)
        ...
        if checkIfDataExistsOnLocal(self.data_dir):
            ...
            if type == "rnn":
                rnnpr.process_audio(process_all=True, window_length=window_length, overlap=overlap,
                                    sampling_rate=sampling_rate, num_segments=num_segments)
            else:
                pr.process_audio(process_all=True, window_length=window_length, overlap=overlap,
                                 sampling_rate=sampling_rate, num_segments=num_segments)
